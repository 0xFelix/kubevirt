...
var _ = Describe("[sig-compute][virtctl]SCP", decorators.SigCompute, func() {
	...
	DescribeTable("should copy a local file back and forth", func(copyFn func(string, string, bool)) {
		...
		Entry("using the native scp method", decorators.Native, copyNative),
		Entry("using the local scp method with --local-ssh flag", decorators.Native, copyLocal(true)),
		Entry("using the local scp method without --local-ssh flag", decorators.ExcludeNative, copyLocal(false)),
	)
	...
	It("local-ssh flag should be unavailable in virtctl binary", decorators.ExcludeNative, func() {
