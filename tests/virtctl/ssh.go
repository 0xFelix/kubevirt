...
var _ = Describe("[sig-compute][virtctl]SSH", decorators.SigCompute, func() {
	...
	DescribeTable("should succeed to execute a command on the VM", func(cmdFn func(string)) {
		...
		Entry("using the native ssh method", decorators.Native, cmdNative),
		Entry("using the local ssh method with --local-ssh flag", decorators.Native, cmdLocal(true)),
		Entry("using the local ssh method without --local-ssh flag", decorators.ExcludeNative, cmdLocal(false)),
	)
