...
var _ = Describe("[rfe_id:1177][crit:medium][vendor:cnv-qe@redhat.com][level:component][sig-compute]VirtualMachine", func() {
	...
	Context("An invalid VirtualMachine given", func() {
		...
		It("[test_id:1519]should reject POST if validation webhoook deems the spec is invalid", func() {
			...
			result := virtClient.RestClient().Post().Resource("virtualmachines").Namespace(util.NamespaceTestDefault).Body(vm).Do(context.Background())
	...
	Context("A valid VirtualMachine given", func() {
		...
		It("[test_id:1522]should remove owner references on the VirtualMachineInstance if it is orphan deleted", func() {
			...
			err = virtClient.VirtualMachine(vm.Namespace).Delete(vm.Name, &k8smetav1.DeleteOptions{PropagationPolicy: &orphanPolicy})
			Expect(err).ToNot(HaveOccurred())
		...
		It("[test_id:1523]should recreate VirtualMachineInstance if it gets deleted", func() {
			...
			err = virtClient.VirtualMachineInstance(vm.Namespace).Delete(vm.Name, &k8smetav1.DeleteOptions{})
			Expect(err).ToNot(HaveOccurred())
		...
		It("[test_id:1524]should recreate VirtualMachineInstance if the VirtualMachineInstance's pod gets deleted", func() {
			...
			vm := createVM(virtClient, libvmi.NewCirros())
			vm = startVM(virtClient, vm)
			firstVMI, err := virtClient.VirtualMachineInstance(vm.Namespace).Get(vm.Name, &k8smetav1.GetOptions{})
			Expect(err).ToNot(HaveOccurred())
			...
			curVMI, err := virtClient.VirtualMachineInstance(vm.Namespace).Get(vm.Name, &k8smetav1.GetOptions{})
			Expect(err).ToNot(HaveOccurred())
			pods, err = virtClient.CoreV1().Pods(vm.Namespace).List(context.Background(), tests.UnfinishedVMIPodSelector(curVMI))
			Expect(err).ToNot(HaveOccurred())
			Expect(pods.Items).To(HaveLen(1))
			curPod := pods.Items[0]
			Expect(curPod.Name).ToNot(Equal(firstPod.Name))
		})
		...
		DescribeTable("should report an error status when VM scheduling error occurs", func(unschedulableFunc func(vmi *v1.VirtualMachineInstance)) {
			...
			Eventually(ThisVM(vm), 300*time.Second, 1*time.Second).Should(Field(Equal(v1.VirtualMachineStatusUnschedulable), "Status", "PrintableStatus"))
		},
		...
		It("[test_id:6869]should report an error status when image pull error occurs", func() {
			...
			const times = 2
			for i := 0; i < times; i++ {
				Eventually(ThisVM(vm), 300*time.Second, 1*time.Second).Should(Field(Equal(v1.VirtualMachineStatusErrImagePull), "Status", "PrintableStatus"))
				Eventually(ThisVM(vm), 300*time.Second, 1*time.Second).Should(Field(Equal(v1.VirtualMachineStatusImagePullBackOff), "Status", "PrintableStatus"))
			}
		})
		...
		DescribeTable("should report an error status when a VM with a missing PVC/DV is started", func(vmiFunc func() *v1.VirtualMachineInstance, status v1.VirtualMachinePrintableStatus) {
			...
			Eventually(ThisVM(vm), 300*time.Second, 1*time.Second).Should(Field(Equal(status), "Status", "PrintableStatus"))
		},
			Entry(
		...
		It("[test_id:7679]should report an error status when data volume error occurs", func() {
			...
			Eventually(ThisVM(vm), 300*time.Second, 1*time.Second).Should(Field(Equal(v1.VirtualMachineStatusDataVolumeError), "Status", "PrintableStatus"))
		})
		...
		Context("Using virtctl interface", func() {
			It("[test_id:1529]should start a VirtualMachineInstance once", func() {
				...
				Eventually(ThisVM(vm), 360*time.Second, 1*time.Second).Should(Field(BeTrue(), "Status", "Ready"))
			...
			It("[test_id:1530]should stop a VirtualMachineInstance once", func() {
				...
				Eventually(ThisVM(vm), 360*time.Second, 1*time.Second).Should(And(Field(BeFalse(), "Status", "Created"), Field(BeFalse(), "Status", "Ready")))
			...
			It("[test_id:6310]should start a VirtualMachineInstance in paused state", func() {
				...
				By("Getting the status of the VM")
				Eventually(ThisVM(vm), 360*time.Second, 1*time.Second).Should(Field(BeTrue(), "Status", "Created"))
			...
			It("[test_id:3007]Should force restart a VM with terminationGracePeriodSeconds>0", func() {
				...
				vm := createVM(virtClient, libvmi.NewFedora(
			...
			It("Should force stop a VMI", func() {
				By("getting a VM with high TerminationGracePeriod")
				vm := createVM(virtClient, libvmi.New(
					libvmi.WithResourceMemory("128Mi"),
				...
				lw, err := virtClient.VirtualMachineInstance(vm.Namespace).Watch(metav1.ListOptions{})
				Expect(err).ToNot(HaveOccurred())
			...
			Context("Using RunStrategyAlways", func() {
				It("[test_id:3163]should stop a running VM", func() {
					...
					vm := createCirrosWithRunStrategy(virtClient, v1.RunStrategyAlways)
				...
				It("[test_id:3164]should restart a running VM", func() {
					...
					Eventually(ThisVM(vm), 240*time.Second, 1*time.Second).Should(Field(BeEmpty(), "Status", "StateChangeRequests"))
				})
			...
			Context("Using RunStrategyRerunOnFailure", func() {
				It("[test_id:2186] should stop a running VM", func() {
					...
					vm := createCirrosWithRunStrategy(virtClient, v1.RunStrategyRerunOnFailure)
				...
				It("[test_id:2188] should not remove a succeeded VMI", func() {
					...
					Eventually(ThisVMIWith(vm.Namespace, vm.Name), 240*time.Second, 1*time.Second).Should(HaveSucceeded())
			...
			Context("Using RunStrategyHalted", func() {
				It("[test_id:2037] should start a stopped VM", func() {
					...
					vm := createCirrosWithRunStrategy(virtClient, v1.RunStrategyHalted)
			...
			Context("Using RunStrategyOnce", func() {
				It("[Serial] Should leave a failed VMI", Serial, func() {
					...
					vm := createCirrosWithRunStrategy(virtClient, v1.RunStrategyOnce)
					...
					By("Ensuring the VirtualMachineInstance remains stopped")
					...
					Consistently(ThisVM(vm), 60*time.Second, 5*time.Second).Should(Field(BeFalse(), "Status", "Ready"))
				})
			...
			Context("Using RunStrategyManual", func() {
				It("[test_id:2036] should start", func() {
					...
					vm := createCirrosWithRunStrategy(virtClient, v1.RunStrategyManual)
				...
				It("[test_id:2035] should restart", func() {
					...
					Eventually(ThisVM(vm), 240*time.Second, 1*time.Second).Should(Field(BeFalse(), "Status", "Created"))
				...
				DescribeTable("with a failing VMI and the kubevirt.io/keep-launcher-alive-after-failure annotation", func(keepLauncher string) {
					...
					Eventually(ThisVM(vm), 160, 1).Should(Field(Equal(v1.VirtualMachineStatusStarting), "Status", "PrintableStatus"))
	...
	Context("[rfe_id:273]with oc/kubectl", func() {
		...
		It("[test_id:264]should create and delete via command line", func() {
			...
			By("Invoking virtctl start")
			virtctl := clientcmd.NewRepeatableVirtctlCommand(virtctl.COMMAND_START, "--namespace", vm.Namespace, vm.Name)
			err = virtctl()
			Expect(err).ToNot(HaveOccurred())
		...
		Context("should not change anything if dry-run option is passed", func() {
			...
			DescribeTable("in stop command", func(flags ...string) {
				...
				originalVMI, err := virtClient.VirtualMachineInstance(vm.Namespace).Get(vm.Name, &k8smetav1.GetOptions{})
				Expect(err).ToNot(HaveOccurred())

				By("Getting current vm instance")
				originalVM, err := virtClient.VirtualMachine(vm.Namespace).Get(vm.Name, &k8smetav1.GetOptions{})
				Expect(err).ToNot(HaveOccurred())
				...
				vm, err = virtClient.VirtualMachine(vm.Namespace).Get(vm.Name, &k8smetav1.GetOptions{})
				Expect(err).ToNot(HaveOccurred())
				Expect(vm.Spec.Running).To(BeEquivalentTo(originalVM.Spec.Running))
				actualRunStrategy, err := vm.RunStrategy()
				Expect(err).ToNot(HaveOccurred())
				...
				vmi, err := virtClient.VirtualMachineInstance(vm.Namespace).Get(vm.Name, &k8smetav1.GetOptions{})
				Expect(err).ToNot(HaveOccurred())
				Expect(vmi.Spec.TerminationGracePeriodSeconds).To(BeEquivalentTo(originalVMI.Spec.TerminationGracePeriodSeconds))
				Expect(vmi.Status.Phase).To(BeEquivalentTo(originalVMI.Status.Phase))
			},
			...
			It("[test_id:7528]in restart command", func() {
				...
				vmi, err := virtClient.VirtualMachineInstance(vm.Namespace).Get(vm.Name, &k8smetav1.GetOptions{})
				Expect(err).ToNot(HaveOccurred())
				Expect(originalVMI.ObjectMeta.CreationTimestamp).To(Equal(vmi.ObjectMeta.CreationTimestamp))
				Expect(originalVMI.ObjectMeta.UID).To(Equal(vmi.ObjectMeta.UID))
				Expect(vmi.ObjectMeta.DeletionTimestamp).To(BeNil())

				By("Checking that VM is running")
				stdout, _, err := clientcmd.RunCommand(k8sClient, "describe", "vmis", vm.Name)
				Expect(err).ToNot(HaveOccurred())
				Expect(vmRunningRe.FindString(stdout)).ToNot(Equal(""), "VMI is not Running")
			})
		})
		...
		It("[test_id:232]should create same manifest twice via command line", func() {
			...
			By("Deleting VM using k8s client binary")
			_, _, err = clientcmd.RunCommand(k8sClient, "delete", "vm", vm.Name)
			Expect(err).ToNot(HaveOccurred())
	...
	Context("crash loop backoff", func() {
		It("should backoff attempting to create a new VMI when 'runStrategy: Always' during crash loop.", func() {
			...
			Eventually(func() error {
				...
				_, err = virtClient.VirtualMachine(vm.Namespace).Update(vm)
				return err
...
func createCirrosWithRunStrategy(virtClient kubecli.KubevirtClient, runStrategy v1.VirtualMachineRunStrategy) *v1.VirtualMachine {
	vm := tests.NewRandomVirtualMachine(libvmi.NewCirros(
...
func startVM(virtClient kubecli.KubevirtClient, vm *v1.VirtualMachine) *v1.VirtualMachine {
	...
	Eventually(ThisVM(vm), 300*time.Second, 1*time.Second).Should(Exist())
...
func stopVM(virtClient kubecli.KubevirtClient, vm *v1.VirtualMachine) *v1.VirtualMachine {
	...
	Eventually(ThisVM(vm), 300*time.Second, 1*time.Second).Should(Field(BeFalse(), "Status", "Ready"))
