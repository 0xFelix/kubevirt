...
var _ = Describe("[rfe_id:1177][crit:medium][vendor:cnv-qe@redhat.com][level:component][sig-compute]VirtualMachine", func() {
	...
	Context("A valid VirtualMachine given", func() {
		...
		DescribeTable("[test_id:1521]should remove VirtualMachineInstance once the VM is marked for deletion", func(createTemplate vmiBuilder) {
			template, dv := createTemplate()
			defer libstorage.DeleteDataVolume(&dv)
			newVM := createVM(virtClient, template)
			newVM = startVM(newVM)
			// Delete it
			...
			Eventually(ThisVMIWith(newVM.Namespace, newVM.Name), 300*time.Second, 1*time.Second).ShouldNot(Exist())
		},
			Entry("with ContainerDisk", newVirtualMachineInstanceWithContainerDisk),
			Entry("[Serial][storage-req]with Filesystem Disk", Serial, newVirtualMachineInstanceWithFileDisk),
			Entry("[Serial][storage-req]with Block Disk", Serial, newVirtualMachineInstanceWithBlockDisk),
		...
		Context("Using virtctl interface", func() {
			It("[test_id:1529]should start a VirtualMachineInstance once", func() {
				...
				Eventually(ThisVM(newVM), 360*time.Second, 1*time.Second).Should(Field(BeTrue(), "Status", "Ready"))

				By("Getting the running VirtualMachineInstance")
				Eventually(ThisVMIWith(newVM.Namespace, newVM.Name), 120*time.Second, 1*time.Second).Should(BeRunning())
			...
			It("[test_id:1530]should stop a VirtualMachineInstance once", func() {
				...
				Eventually(ThisVM(newVM), 360*time.Second, 1*time.Second).Should(And(Field(BeFalse(), "Status", "Created"), Field(BeFalse(), "Status", "Ready")))
			...
			It("[test_id:6310]should start a VirtualMachineInstance in paused state", func() {
				...
				By("Getting the status of the VM")
				Eventually(ThisVM(newVM), 360*time.Second, 1*time.Second).Should(Field(BeTrue(), "Status", "Created"))
