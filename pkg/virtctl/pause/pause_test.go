...
var _ = Describe("Pausing", func() {
	...
	DescribeTable("should pause VMI", func(pauseOptions *v1.PauseOptions) {
		...
		if len(pauseOptions.DryRun) == 0 {
			command = testing.NewRepeatableVirtctlCommand(COMMAND_PAUSE, "vmi", vmName)
		} else {
			command = testing.NewRepeatableVirtctlCommand(COMMAND_PAUSE, "--dry-run", "vmi", vmName)
		}
	...
	DescribeTable("should pause VM", func(pauseOptions *v1.PauseOptions) {
		...
		var command func() error
		if len(pauseOptions.DryRun) == 0 {
			command = testing.NewRepeatableVirtctlCommand(COMMAND_PAUSE, "vm", vmName)
		} else {
			command = testing.NewRepeatableVirtctlCommand(COMMAND_PAUSE, "--dry-run", "vm", vmName)
		}
		Expect(command()).To(Succeed())
	},
		Entry("", &v1.PauseOptions{}),
